                    ...
                    <form action="{{ route('dopes.update', $dope) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="serial_number" id="serial_number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('serial_number', $dope->serial_number) }}">
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="quantity" id="quantity" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('quantity', $dope->quantity) }}" required>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="aircraft_registration" id="aircraft_registration" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    @foreach($aircrafts as $aircraft)
                                        <option value="{{ $aircraft->name }}" {{ old('aircraft_registration', $dope->aircraft_registration) == $aircraft->name ? 'selected' : '' }}>{{ $aircraft->name }}</option>
                                    @endforeach
                                </select>
                            </div>
