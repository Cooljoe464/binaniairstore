                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($rotables as $rotable)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($rotable->quantity)->getColorClass() }}">{{ $rotable->quantity }}</span></td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $rotable->aircraft_registration }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ Str::limit($rotable->remark, 15) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $rotable->receivedBy->name ?? 'N/A' }}</td>
            ...
            @foreach ($rotables as $rotable)
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <div class="mt-2 text-sm text-gray-600">
                        ...
                        <div><strong>Qty:</strong> <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($rotable->quantity)->getColorClass() }}">{{ $rotable->quantity }}</span></div>
                        <div><strong>Aircraft Registration:</strong> {{ $rotable->aircraft_registration }}</div>
                        <div><strong>Remark:</strong> {{ Str::limit($rotable->remark, 30) }}</div>
                        <div><strong>Received By:</strong> {{ $rotable->receivedBy->name ?? 'N/A' }}</div>
