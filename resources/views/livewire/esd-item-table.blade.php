                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($esdItems as $esdItem)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($esdItem->received_quantity)->getColorClass() }}">{{ $esdItem->received_quantity }}</span></td>
                            <td class="px-6 py-4 whitespace-nowrap"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($esdItem->accepted_quantity)->getColorClass() }}">{{ $esdItem->accepted_quantity }}</span></td>
                            <td class="px-6 py-4 whitespace-nowrap"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($esdItem->binned_quantity)->getColorClass() }}">{{ $esdItem->binned_quantity }}</span></td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $esdItem->ak_reg }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ Str::limit($esdItem->remark, 15) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $esdItem->storeOfficer->name ?? 'N/A' }}</td>
            ...
            @foreach ($esdItems as $esdItem)
                <div class="bg-white p-4 rounded-lg shadow">
                    ...
                    <div class="mt-2 text-sm text-gray-600">
                        ...
                        <div><strong>Received Qty:</strong> <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($esdItem->received_quantity)->getColorClass() }}">{{ $esdItem->received_quantity }}</span></div>
                        <div><strong>Accepted Qty:</strong> <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($esdItem->accepted_quantity)->getColorClass() }}">{{ $esdItem->accepted_quantity }}</span></div>
                        <div><strong>Binned Qty:</strong> <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ App\Enums\QuantityStatus::fromQuantity($esdItem->binned_quantity)->getColorClass() }}">{{ $esdItem->binned_quantity }}</span></div>
                        <div><strong>AK Reg:</strong> {{ $esdItem->ak_reg }}</div>
                        <div><strong>Remark:</strong> {{ Str::limit($esdItem->remark, 30) }}</div>
                        <div><strong>Store Officer:</strong> {{ $esdItem->storeOfficer->name ?? 'N/A' }}</div>
