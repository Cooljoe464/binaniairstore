                    ...
                    <form action="{{ route('consumables.update', $consumable) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="received_quantity" id="received_quantity" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('received_quantity', $consumable->received_quantity) }}" required>
                            </div>

                            <div class="mb-4">
                                <label for="accepted_quantity" class="block text-sm font-medium text-gray-700">Accepted Quantity</label>
                                <input type="number" name="accepted_quantity" id="accepted_quantity" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('accepted_quantity', $consumable->accepted_quantity) }}" required>
                            </div>

                            <div class="mb-4">
                                <label for="binned_quantity" class="block text-sm font-medium text-gray-700">Binned Quantity</label>
                                <input type="number" name="binned_quantity" id="binned_quantity" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('binned_quantity', $consumable->binned_quantity) }}" required>
                            </div>

                            <div class="mb-4">
                                <label for="ak_reg" class="block text-sm font-medium text-gray-700">AK Reg</label>
                                <input type="text" name="ak_reg" id="ak_reg" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('ak_reg', $consumable->ak_reg) }}" required>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="received_by_id" id="received_by_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('received_by_id', $consumable->received_by_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="store_officer_id" id="store_officer_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    @foreach($storeOfficers as $officer)
                                        <option value="{{ $officer->id }}" {{ old('store_officer_id', $consumable->store_officer_id) == $officer->id ? 'selected' : '' }}>{{ $officer->name }}</option>
                                    @endforeach
                                </select>
                            </div>
